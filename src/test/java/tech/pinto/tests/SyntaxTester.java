...
//	private final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(this.getClass());
	...
	public void nestedIndex() throws Exception {
		Table t = pinto.evaluate(" [c=10] range [3,2,z= 99 .5 [1] only ] only eval").get(0);
	...
	public void inlineFunction() throws Exception {
		Table t = pinto.evaluate(" 1 2 3 ([0] 1 + 1 +) 4 eval").get(0);
		...
		t = pinto.evaluate("{test:1 2} [1] ( [0] rolling sum) eval").get(0);
		...
		double[][] d = pinto.evaluate("{test: 1 2} [test+] ( [0] rolling sum) eval").get(0).toColumnMajorArray();
	...
	public void extraCommasInHeaderLiteral() throws Exception {
		Table t = pinto.evaluate("{x : 1 2 3 [0,1] only, y : 5} eval").get(0);
