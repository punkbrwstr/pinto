...
public class MoonPhase {
	...
	public static int days_of_february(int year) {
		...
		if ((year > greg[YEAR])
			...
			if (orthodox_calendar)
				day = ((year & 3) != 0) ? 28
						: ((!((year % 100) != 0)) ? (((year % 9) == 2 || (year % 9) == 6) ? 29 : 28) : 29);
			else
	...
	public static double phase(double julian_date) {
		...
		moon_evection = 1.2739
				* Math.sin(TORAD(2.0 * (moon_mean_longitude - sun_geocentric_elong) - moon_mean_anomaly));
	...
	public static double calendarToJD(Calendar cal) {
		...
		return (((long) (365.25 * (y + 4716))) + ((int) (30.6001 * (m + 1))) + mday + b - 1524.5)
	...
	public String getMoonAgeAsDays() {
		...
		return "" + aom_d + (aom_d == 1 ? " day, " : " days, ") + aom_h + (aom_h == 1 ? " hour, " : " hours, ") + aom_m
