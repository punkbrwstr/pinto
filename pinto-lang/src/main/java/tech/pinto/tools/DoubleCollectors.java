...
public enum DoubleCollectors implements Supplier<DoubleCollector>{
	...
	changepct(dc -> dc.getLast() / dc.getFirst() - 1.0d),
	changelog(dc -> Math.log(dc.getLast()) - Math.log(dc.getFirst())),
	
	/* "true" window ones */
	average(dc -> dc.getSum() / (double) dc.count()),
	...
	stdev(dc -> Math.sqrt(var.getFunction().applyAsDouble(dc))),
	stdevp(dc -> Math.sqrt(varp.getFunction().applyAsDouble(dc))),
